...
import { accentCssVar, type ChartMode, type RankFlowSeries } from "./types";
...
/** Bump chart: rank positions (1 = top) flowing across quarters. Ranks are
 * discrete data — "live" mode here means the lanes keep gently lerping
 * toward their target rank instead of snapping, not a fabricated wobble. */
    ...
    (ctx, width, height) => {
      ...
      series.forEach((serie, seriesIndex) => {
        ...
        const points = serie.ranks.map((_, col) => {
          const target = stateRef.current.target[seriesIndex][col];
          const next =
            effectiveMode === "static" || reducedMotion
              ? target
              : lerp(stateRef.current.current[seriesIndex][col], target, 0.07);
