...
export function ErrorState({ title, code, message, action, className }: ErrorStateProps) {
  return (
    <div
      className={cn(
    ...
    >
      <strong className="font-display text-c1 text-lg font-semibold">{title}</strong>
      {code && (
        <span className="font-mono text-terra3 text-[9px] tracking-[0.2em] uppercase">
          {code}
        </span>
      )}
      <span className="text-c3 max-w-md text-sm leading-relaxed">{message}</span>
      {action}
    </div>
  );
}
