...
2. Read the relevant `docs/product/roadmap.md` episode description and the invariants registry (`docs/architecture/invariants.md`) before editing anything. Plans and invariants can change between sessions — treat the files as the source of truth, not memory of a prior conversation.
...
10. Mark the episode's `**Status.**` line in `docs/product/roadmap.md`, fill the task checklist with `+`/`-`, and write the final report.
