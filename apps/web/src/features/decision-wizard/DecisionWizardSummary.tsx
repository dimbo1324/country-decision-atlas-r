  ...
  return (
    <div className="decisionWizardResult" data-testid="decision-wizard-result">
      <div className="decisionWizardResultTop">
        ...
        <span>
          {labels.scenario}: <strong>{recommendation.recommended_scenario_slug}</strong>
        </span>
        <span>
          {labels.persona}:{" "}
          <strong>{recommendation.recommended_persona_slug ?? labels.noPersona}</strong>
        </span>
      </div>
      <div className="decisionWizardSummaryBlock">
        <strong>{labels.explanation}</strong>
        <ul className="pointsList" data-testid="decision-wizard-explanation">
      ...
      {recommendation.warnings.length > 0 && (
        <div className="decisionWizardSummaryBlock">
          ...
          <ul className="warningsList" data-testid="decision-wizard-warnings">
            {recommendation.warnings.map((warning) => (
              <li key={warning} className="warningItem">
      ...
      <p className="formHint" data-testid="decision-wizard-manual-note">
        {labels.manualNote}
      </p>
    </div>
  );
