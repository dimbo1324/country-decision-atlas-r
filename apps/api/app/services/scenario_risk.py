...
SCENARIO_RELEVANCE = {
    "relocation_residence": {
        ...
        "affected_groups": {"migrants", "residents", "families", "remote_workers"},
    },
    "permanent_residence_citizenship": {
        "signal_types": {"migration", "residence", "citizenship", "rule_of_law"},
        "affected_groups": {"migrants", "residents", "families"},
    },
    "low_budget_living": {
        "signal_types": {"cost_of_living", "tax", "banking", "healthcare", "safety"},
        "affected_groups": {"low_income", "migrants", "families"},
    },
    "business_self_employment": {
        "signal_types": {"business", "tax", "banking", "property", "rule_of_law"},
    ...
    "safety_political_risk": {
        ...
        "affected_groups": {"migrants", "activists", "journalists", "residents"},
    },
}
LEVEL_POINTS = {
    "low": 5.0,
...
def compute_scenario_specific_risk_score(
    ...
    if scenario_slug not in SCENARIO_RELEVANCE:
        return _insufficient(scenario_slug, rows, window_days, 0, len(rows), 0.0)
    ...
    risk_delta = sum(
        _signal_points(row, relevance) for relevance, row in scored if relevance > 0
    )
...
def _insufficient(
    ...
    relevant_rows = [
        row
        for row in rows
        if scenario_slug in SCENARIO_RELEVANCE and _relevance(row, scenario_slug) > 0
    ]
    evidence_count = unique_count(relevant_rows, "evidence_item_id")
    source_count = unique_count(relevant_rows, "source_id")
    return PlatformMetricComputation(
...
def _weak_group_match(groups: set[str], relevant: set[str]) -> bool:
    return any(group in item or item in group for group in groups for item in relevant)
...
def _freshness(rows: Sequence[Mapping[str, Any]]) -> str:
    if not rows:
        return "unknown"
    freshest_age = min(
        (age for row in rows if (age := days_old(row.get("event_date"))) is not None),
        default=None,
    )
