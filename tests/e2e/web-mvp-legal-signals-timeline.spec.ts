...
test.describe("legal signals timeline", () => {
  test("timeline renders year groups and country events", async ({ page }) => {
    ...
    await expect(eventCards.getByText(/уругвай|uruguay/i).first()).toBeVisible();
    await expect(
      eventCards.getByText(/положительное|негативное|нейтральное|смешанное/i).first(),
    ).toBeVisible();
  ...
  test("legend and source traceability render", async ({ page }) => {
    ...
    await expect(page.getByText(/источник:/i).first()).toBeVisible({ timeout: 10_000 });
    await expectNoAppCrash(page);
  });
