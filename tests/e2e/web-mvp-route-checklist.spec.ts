...
test.describe("Route practical checklist", () => {
  test("open country, open route detail, see practical checklist", async ({ page }) => {
  ...
  test("checklist source link is visible where seeded", async ({ page }) => {
    ...
    await expect(page.getByTestId("route-checklist-source-link").first()).toBeVisible();
    await expectNoAppCrash(page);
  });

  test("route without checklist shows empty state, not a crash", async ({ page }) => {
    await page.goto(e2eRoutes.country("russia", "ru"));
    await expectPageReady(page);
