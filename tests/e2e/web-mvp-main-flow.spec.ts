...
test("main MVP user flow: home → countries → Russia → Uruguay → decision → result → country card", async ({
  ...
  await expect(page.getByRole("link", { name: /run decision/i }).first()).toBeVisible();
  ...
  await page.getByRole("link", { name: "View country card" }).nth(1).click();
  await expect(page.locator("h1")).toBeVisible();
  await expect(page.getByText(/scenario scores/i)).toBeVisible();
  await expectNoAppCrash(page);
  ...
  await expect(page.getByRole("combobox", { name: /origin country/i })).toBeVisible();
