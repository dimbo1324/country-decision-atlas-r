...
- `apps/web` — Next.js frontend (App Router, next-intl three-language
  interface — en/ru/es, English default). Design system in `packages/ui`;
  Vitest for unit tests, Storybook + MSW for component interaction tests,
  a separate `playwright.visual.config.ts` for visual regression (kept out
  of the main E2E gate — OS-dependent pixel diffing).
- `apps/notifier` — Go gRPC notifier service (Kafka consumer → Telegram;
  Mongo for derived state). Bot replies and event notifications are
  locale-aware (`internal/locale`), driven by the sender's Telegram
  `language_code`.
- `apps/worker` — Python CLI for batch jobs (`translation-jobs`) run
  outside the request path; not yet wired into
  `dev_tools_scripts_runner.py` or CI.
